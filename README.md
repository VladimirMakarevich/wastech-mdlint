...
- `graph` prints the context graph to stdout: clusters, hubs, reading order, and the coverage signal as `human` text (default); the deterministic `{ nodes, edges, components, readingOrder, coverage }` shape as `json`; or a `mermaid`/`dot` diagram for rendering elsewhere.
