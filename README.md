...
- **Every run already excludes 11 noise globs** — `node_modules`, `dist`, `build`, `.git`, any dot-directory, and friends, at any depth — before you write anything, and your own `exclude` is **appended** to them rather than replacing them. So `"exclude": []` is not an opt-out and deleting an entry cannot remove a default; negate one instead. Full list: [what is excluded before you write anything](docs/guide/configuration.md#what-is-excluded-before-you-write-anything).
