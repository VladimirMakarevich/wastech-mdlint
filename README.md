...
| --- | --- | --- | --- |
| `lint` | Lint ad-hoc Markdown content against an explicit set of rules. Does not load project config; file-resolving rules such as REF-001/REF-003 and SEC-003 may probe or read paths inside the server's working directory; an absolute path or a `..`-escaping relative path is rejected rather than followed. | yes | yes |
