...
describe.skipIf(process.platform === "win32")("run-npm-windows.sh", () => {
  let sandbox: string;
  let checkout: string;
  let stubDir: string;
  let runCounter = 0;
  ...
  function run(
    ...
    const result = spawnSync("bash", [path.join(checkout, "scripts", script)], {
      cwd: sandbox,
      env: {
        ...process.env,
  ...
  // Every test below spawns a real bash script, which in turn spawns the stubs — three process
  // creations per `run()`, and `verify-wsl.sh` makes four such rounds. In isolation each finishes
  // in about two seconds, but process creation competes with the rest of the suite for the host,
  // and under a fully parallel run the same work measured over eleven. The default per-test budget
  // therefore failed these on load rather than on behavior, which is the least useful kind of red.
  // The number is a ceiling for a hang, not an expectation of the runtime.
  ...
  it(
    "builds no shell command line and suppresses no engine check",
    () => {
  ...
  it(
    "verify-wsl.sh still runs its four steps in order",
    () => {
  ...
  it(
    "propagates the Windows exit code to its caller",
    () => {
  ...
  it(
    "refuses a WSL-filesystem checkout by name rather than running in C:\\Windows",
    () => {
  ...
  it("leaves the engines floor to the root .npmrc", () => {
    ...
    expect(
      ...
      "The root .npmrc is what makes `engines.node` binding for our own installs, and " +
        "scripts/run-npm-windows.sh dropped `--engine-strict=false` on that basis. Removing " +
        "it puts the Windows-from-WSL toolchain back outside the declared floor.",
    ).toBe(true);
