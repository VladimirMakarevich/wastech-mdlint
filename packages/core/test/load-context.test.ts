...
async function fixtureRepo(files: Record<string, string>): Promise<string> {
  const root = await mkdtemp(
    path.join(os.tmpdir(), "wastech-mdlint-load-context-"),
  );
  tempDirs.push(root);
...
describe("loadContext", () => {
  it("keys documents by repo-relative POSIX path and builds a matching graph", async () => {
    ...
    expect(context.graph.nodes.map((node) => node.path).sort()).toEqual([
      "a.md",
      "b.md",
    ]);
  ...
  it("threads settings.idRef into the graph so id-ref edges materialize", async () => {
    ...
    expect(context.graph.edges.some((edge) => edge.type === "id-ref")).toBe(
      true,
    );
  });
