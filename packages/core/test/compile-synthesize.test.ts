...
function dependenciesSectionPrefix(content: string): string {
  ...
  return content.slice(
    start,
    commandIndex === -1 ? content.length : commandIndex,
  );
}
...
describe("synthesize", () => {
    ...
    const twoDocs = input({
      ...
      profiles: new Map([
        [
          ...
          profile({
            role: "entry",
            referencesTo: [{ from: "a.md", to: "b.md", type: "link", line: 1 }],
          }),
        ],
        [
          "b.md",
          profile({
            role: "leaf",
            referencedBy: [{ from: "a.md", to: "b.md", type: "link", line: 1 }],
          }),
        ],
      ]),
      analysis: analysis({
        readingOrder: ["a.md", "b.md"],
        components: [["a.md", "b.md"]],
      }),
    });
  ...
  it("renders frontmatter that validates against the SKILL.md schema", () => {
    const result = synthesize(
      input({ skill: { name: "My Skill", description: "Does things" } }),
    );

    expect(
      result.skillContent.startsWith(
  ...
  it("throws when name or description is empty", () => {
    ...
    expect(() =>
      synthesize(input({ skill: { name: "x", description: "" } })),
    ).toThrow();
  });
  ...
  it.each(Object.entries(commandBlocks))(
    "renders the locked %s command block verbatim",
    (preset, block) => {
      const result = synthesize(
        input({ commandPreset: preset as CompileCommandPreset }),
      );
    ...
    const profiles = new Map([
      [
        ...
        profile({
          role: "entry",
          idPattern: "REQ-NNN",
          tableSchemas: [{ headers: ["ID"], line: 1 }],
        }),
      ],
      [
        "table.md",
        profile({
          role: "hub",
          tableSchemas: [{ headers: ["Name"], line: 1 }],
        }),
      ],
  ...
  it("renders '(no documents found)' in Document Architecture for an empty corpus", () => {
    const result = synthesize(
      input({ documentPaths: [], profiles: new Map() }),
    );
  ...
  it.each(["architecture", "rules", "dependencies"] as const)(
    ...
    (gatedOff) => {
      ...
      expect(block).not.toContain(
        SECTION_HEADINGS[gatedOff].replace(/^##+\s*/, ""),
      );
  ...
  it("still reports an empty corpus honestly when there are truly no documents", () => {
    const result = synthesize(
      input({ documentPaths: [], analysis: analysis() }),
    );
    const readingOrderBlock = result.skillContent.slice(
      result.skillContent.indexOf("### Reading Order"),
  ...
  it("renders over-budget entrypoints with their numbers", () => {
    const result = synthesize(
      input({
        budget: budget({
          ...
          entrypointsOverBudget: [
            { path: "CLAUDE.md", totalTokens: 120, maxTokens: 100 },
          ],
        }),
      }),
    );
    ...
    expect(result.skillContent).toContain(
      "`CLAUDE.md`: 120 estimated tokens exceeds 100 (20.0% over).",
    );
  });

  it("renders 'within budget' when entrypoints are configured but none are over", () => {
    const result = synthesize(
      input({ budget: budget({ llm001Enabled: true, entrypointsMatched: 2 }) }),
    );
    expect(result.skillContent).toContain(
      "All configured entrypoints are within budget.",
    );
  });
  ...
  it("renders a truthful zero-match state when LLM-001 is enabled but its glob matches no files", () => {
    ...
    const result = synthesize(
      input({ budget: budget({ llm001Enabled: true, entrypointsMatched: 0 }) }),
    );
  ...
  it("always renders the Context Budget section even when every other section is gated off", () => {
    ...
    const result = synthesize(
      input({ sections, budget: budget({ corpusTokenEstimate: 42 }) }),
    );
  ...
  it("computes metadata counts from input sizes", () => {
    const ruleGroups: RuleDescriptionGroup[] = [
      ...
      },
      {
        category: "SEC",
        label: "Sections",
        rules: [{ id: "SEC-001", description: "d3" }],
      },
    ];
  ...
  it("changes the content hash when ruleCount changes even though Document Rules is gated off", () => {
        ...
        ruleGroups: [
          {
            category: "TBL",
            label: "Table Structure",
            rules: [{ id: "TBL-001", description: "d" }],
          },
        ],
      }),
    );
    ...
    expect(fewRules.metadata.contentHash).not.toBe(
      moreRules.metadata.contentHash,
    );
  });
  ...
  it("escapes a document path containing a pipe in the Document Architecture table", () => {
    ...
    expect(result.skillContent).toContain(
      "| docs/a\\|b.md | isolated | narrative |",
    );
  });
