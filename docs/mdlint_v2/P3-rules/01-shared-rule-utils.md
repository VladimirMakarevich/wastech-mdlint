...
- **Previous:** [P2.07 — First rules + `lint`](../P2-rule-engine/07-first-rules-lint-command.md) proved the engine path on a couple of rules.
...
1. `glob-match` — picomatch matcher with `{ dot: true }` (so `.claude/` etc. are matched); the shared `files`/`exclude` scoping helper ([R7](../requirements/02-rules-engine.md)).
...
5. `site-router` — `resolveRoutedUrl(url, router)` with the Starlight preset; fed by `settings.siteRouter` ([C5](../requirements/01-configuration.md)) with per-rule override.

## Decisions applied

- [R7](../requirements/02-rules-engine.md) shared scoping · [C5](../requirements/01-configuration.md) site-router from settings.
...
P3.02–P3.07 compose rules from primitives + these utils with no duplicated glob/line/regex logic.
