...
Implement checklist completeness and content-quality rules over the checklist/content primitives.
...
1. CTX-001 composes `noPlaceholders` + empty-section detection via `extract-section-body`. **Placeholder set (locked 2026-07-02, audit 3.1):**
   - Default set: `["TBD", "TODO", "WIP", "FIXME", "N/A"]` (these five, exactly).
   - The `placeholders` option **extends** the default set (union — honoring "extensible"); it does not replace it.
   - **Matching is case-insensitive and whole-body:** a section is flagged when its trimmed body is empty/whitespace, or equals one of the placeholder tokens (optionally with a trailing `:`). **Not substring** — prose that merely mentions "TODO" is not flagged.
2. CTX-002 composes `allChecked` over `checkItems` (optionally scoped to a section).
3. CTX-003 is project-scope: read glossary table → build alias→canonical map → scan `content` of matched files → warn on alias usage with the canonical replacement.

## Decisions applied

- [R9](../requirements/02-rules-engine.md) presets · [R3](../requirements/02-rules-engine.md) structured findings (suggest canonical via `data`).
...
Content/checklist primitives are validated for `custom` (P3.08); CTX placeholder detection feeds the `-fix` skill policy (P8/S8).
