...
`LintMessage` (additions): `column?`, `endLine?`, `fixable?`, `data?` (offending / expected value, column), `helpUri?` (R3). Existing fields unchanged → JSON output is a superset, safe for current consumers.
