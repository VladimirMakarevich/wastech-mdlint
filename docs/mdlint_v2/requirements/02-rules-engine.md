...
| **R7** | Shared `files`/`exclude` options base for every rule           | ✅ Accepted               | Uniform scoping shape (`fileScopeShape`, `rules/scope.ts`) available to every rule, but mixed in per-rule: identity/whole-corpus rules (REF-001/003/004/005, LLM-001, SIZE-001's own `overrides[].pattern`) intentionally omit it. The P3 task tables are authoritative on which rules take `files?`/`exclude?`. |
