...
| **R7** | Shared `files`/`exclude` options base for every rule           | ✅ Accepted               | Uniform scoping shape (`fileScopeShape`, `rules/scope.ts`) available to every rule, but mixed in per-rule: identity/whole-corpus rules (REF-001/003/004/005, LLM-001, SIZE-001's own `overrides[].pattern`, and `GRP-001` — see [P11.13](../P11-remediation/13-grp-size-hygiene.md)) intentionally omit it. The P3 task tables are authoritative on which rules take `files?`/`exclude?`.          |
...
cannot express a _new kind_ of invariant.
...
- **Graph (P4):** `buildContextGraph` gains the inputs graph rules need to reuse it
  (R5) — as shipped, `siteRouter` and `idRef` only, because a corpus-wide graph is
  what makes every rule reason over the same relationships.
- **Compile (P5):** `describeRules` reads the rule-metadata source (R6) and must
  describe custom rules too.
- **CLI (P3):** `--fix` flag lands with the first `fixable` rule (R2, audit 4.2 — was
  loosely "P6+"); `wastech-mdlint schema` command (C9).
