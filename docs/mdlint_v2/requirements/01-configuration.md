...
- **C7 — diagnostics.** Unknown rule → `did you mean REF-001?`; bad options → exact path (`rules[3].options.idPattern: expected valid RegExp`). Turns the spec's "opaque options" into actionable errors.
