...
- **`lint-files` leaves the zero-config include fallback to core.** The tool only sets `config.include` when an explicit `patterns` arg is given (replacing, not merging); absent it, core's own `include ?? ["**/*.md"]` default applies, so the fallback behavior is provably core's, not reimplemented at the boundary. (The glob is deliberately outside the bold span above: nesting a glob-bearing code span inside one is what corrupted this line before, silently and with the format gate green.)
