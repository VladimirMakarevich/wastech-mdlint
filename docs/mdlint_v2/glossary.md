...
- **`init`** — Zero-to-config bootstrap: scans for doc clusters, re-infers rules against the
  ...
  already exists — the write summary reports whether it already matches or differs, and a
  differing file is only regenerated by removing or renaming it before the next `merge`); a
  `merge` whose existing config is unreadable or would not load (unknown key/rule/options) aborts
  the write rather than produce an invalid or lossy result. `--with-ci-workflow` (under `--yes`
