...
- **`--fix` / `fixable` / `TextEdit`** — The auto-fix path: a rule may expose a `fix()` hook
  ...
  (sort descending, splice). `fixable` marks which rules/findings support it. `applyFixes`
  normalizes every edit's `newText` to the host document's detected newline (`detectNewline`,
  first terminator wins, LF default) and commits each document through an
  **atomic write**, throwing `FixWriteError` — repo-relative path, errno, and the files already
  fixed — on the first document it cannot write. Decision
  [R2](requirements/02-rules-engine.md), [P11.09](P11-remediation/09-atomic-writes.md).
...
- **`init`** — Zero-to-config bootstrap: scans for doc clusters, re-infers rules against the
  ...
  summary to stdout and exits `2` (a deliberate no-write outcome stays `0`).
  `--with-ci-workflow` (under `--yes`
  only) drops the opt-in `.github/workflows/wastech-mdlint.yml`; interactive runs prompt for it
  (default no). See **Init & repo scan** and `generateInitConfig` for the underlying
  scanner/inference/writer. Decisions
...
- **Atomic write** — The single write path every product file goes through
  (`writeFileAtomic` / `writeFilesAtomic`, exported from `core`): content lands in a temp file in
  the target's own directory (`.<basename>.<random>.tmp`, `wx`) and is then `rename`d into place, so
  a failed write can never truncate an existing file. `writeFilesAtomic` stages every temp before
  renaming any of them and returns an `AtomicWriteResult` naming what was and was not committed, in
  the caller's commit order. Scope boundaries: no `fsync` (truncation-safe, not power-loss durable),
  no cross-filesystem fallback (the temp is always beside its target, so `EXDEV` is unreachable),
  and directory creation stays with the caller. Replaced-file mode and a symlinked target are
  preserved. See [P11.09](P11-remediation/09-atomic-writes.md).
