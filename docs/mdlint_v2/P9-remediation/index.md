...
The P0–P8 audit confirmed the architecture is sound and all 24 rules ship, but it surfaced a small set of real behavioral defects and verification gaps that should not go out in a `v1` release: wrong source positions for multi-line imports, a locale-dependent sort on the determinism-critical loader path, a CI that never exercises Windows despite a cross-platform contract, a tool description that misstates filesystem access, an over-promised custom-rule target, and a red-but-unenforced format gate. This phase fixes the code and its verification; [P10](../P10-consistency/index.md) handles the documentation/contract/test-depth findings.
