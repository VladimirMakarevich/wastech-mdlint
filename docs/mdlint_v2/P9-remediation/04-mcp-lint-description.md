...
`packages/mcp-server/src/tools/lint.ts:177` describes the tool as _"Reads no filesystem or
config."_ But `lint.ts:141` sets `rootDir: process.cwd()`, and REF-001/REF-003 resolve relative
...
1. Rewrite the `lint` description to be accurate, e.g. _"Does not read config; for reference
   rules (REF-001/003) it may probe whether relative link/image targets exist, relative to the
   server's working directory."_
