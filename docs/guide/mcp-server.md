...
- The ad-hoc `lint` tool does not load project config. File-resolving rules such as
  [REF-001](rules/REF-001.md)/[REF-003](rules/REF-003.md), [SEC-003](rules/SEC-003.md) and
  [STR-001](rules/STR-001.md) may probe or read paths **inside** the server's working directory; an
