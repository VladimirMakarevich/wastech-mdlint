...
| `lint-files` | Lint the project's Markdown files using the resolved config (or the zero-config `**/*.md` default). An explicit `patterns` argument replaces `include` only — the [default `exclude`](configuration.md#what-is-excluded-before-you-write-anything) still applies, so `patterns` aimed inside `node_modules`, build output or a dot-directory returns an empty corpus. | yes |
