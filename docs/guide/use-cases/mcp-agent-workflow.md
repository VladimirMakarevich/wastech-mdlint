...
**Step 2 — lint the whole project after editing.** Uses the resolved config (or the zero-config `**/*.md` default). `patterns` stands in for `include`, not for the whole corpus scope: the [default `exclude`](../configuration.md#what-is-excluded-before-you-write-anything) still prunes `node_modules`, build output and dot-directories, so a pattern pointed at one of those comes back empty rather than erroring.
