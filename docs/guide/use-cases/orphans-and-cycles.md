...
wastech-mdlint graph . --format human    # clusters, hubs, reading order, excluded, coverage
wastech-mdlint graph . --format mermaid  # a diagram to paste into docs
```

**You get:** cycle and orphan findings ([GRP-001](../rules/GRP-001.md), [GRP-002](../rules/GRP-002.md)) plus a navigable overview from the [`graph`](../context-graph.md) command. The report's [excluded set](../context-graph.md#graph) is what a cycle costs you in practice: those are the documents no reading order can place, so it measures the damage the findings only announce.
