...
**Goal:** before changing a load-bearing doc, see what depends on it; and explore what one entry point pulls in.
...
**You get:** the affected subgraph (with lint narrowed to it) from [`impact`](../context-graph.md), and forward reachability from [`slice`](../cli.md#slice-query). Resolution is exact match only (ID, `#slug`, or path) — a miss is an honest empty result, not an error.
