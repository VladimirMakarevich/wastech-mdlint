...
MCP is stricter than the CLI about what it will name. A failure whose path falls **outside** the analyzed directory, or that reports no path at all (`ENOSPC: no space left on device, write`), comes back as `INTERNAL_ERROR` with a fixed sanitized message instead, while the CLI, writing to your own terminal, keeps the `../` form and the original message. The property that buys is that **no MCP payload names a host path you did not supply yourself**: what the run discovers about the host never widens what the caller already knew. Two paths you do supply come back as given. The tool's `cwd` is echoed as a resolved absolute path by the [`INVALID_INPUT` rejection](mcp-server.md#error-contract) when it does not exist or is not a directory — the broken thing being reported. And a `configPath` that cannot be found is [named relative to that `cwd`](mcp-server.md#error-contract), `../` chain included, or absolute where the two sit on different Windows drives.
