...
- **The dot-directory prune is scan-only, and the draft discloses it per file.** A directory is never excluded from the lint corpus for starting with a dot — only for being a dependency or build tree — so `.claude/skills/` and `.agents/rules/` stay lintable. But the scan still refuses to _propose_ them. That does not mean the config it writes misses them: config globs match dot-segments, so a cluster glob written for an ancestor directory covers a dot-directory nested under it, and a repository whose only Markdown lives in dot-directories gets no cluster at all, an omitted `include`, and the dot-matching default that lints them anyway. Rather than leave you to infer any of this, the draft prints an **Excluded from the scan** block before you confirm, one line per reason, with the hidden class split by what the config beside it will actually lint:

  ```text
  Excluded from the scan:
    hidden directories, not linted: 3 Markdown files in 2 directories whose name starts with a dot — .agents (2), .claude (1). The scan never proposes a dot-directory as a doc cluster, and no include pattern above matches these; add a pattern such as ".agents/**/*.{md,mdx}" to lint them.
    hidden directories, linted anyway: 1 Markdown file in 1 directory whose name starts with a dot — docs/.internal (1). The scan never proposed these as doc clusters, but an include pattern above matches them anyway, so they are in the corpus.
    build and dependency directories: 2 directories skipped by name, contents not counted — node_modules (2).
    gitignored directories: 1 directory skipped, contents not counted — generated-docs (1).
  ```

  Each line states a total and then names its parts with their own counts, so you can add them up rather than take the total on faith; a list longer than five entries ends in a `+N more (M)` tail carrying the total it drops. Only the hidden class names files: it is the one whose contents you plausibly want linted, and the only one cheap to enumerate. The other two say **contents not counted** rather than implying a zero — `init` does not walk a dependency tree to count what it is skipping. A line whose group is empty is omitted, so a repository with no incidentally covered dot-directory sees three lines rather than a zero.

- **`init` never proposes [`SIZE-001`](rules/SIZE-001.md) or [`LLM-001`](rules/LLM-001.md), and the draft says so.** Inference proposes a rule only when it can also propose a working configuration for it, and both of these are budgets: `SIZE-001` needs at least one of `bytes`/`lines`/`tokens`, `LLM-001` needs `entrypoints` **and** `maxTokensPerEntrypoint`. `init` does measure your corpus, but a budget is a choice rather than a measurement — knowing how large every document is says nothing about how large one ought to be — and `LLM-001`'s budget is measured over a whole transitive `@`-import closure, which inference never builds. Any threshold `init` wrote would be invented rather than measured, and you would inherit a limit that looks authoritative and is not. The two are worth adding by hand: they are the LLM context-hygiene rules the README leads with, their pages carry worked examples, and `wastech-mdlint schema` (or the `$schema` your config already points at) will complete and validate the options as you type them. Everything else `init` skips is skipped for the same reason — see the [rule table](../../README.md#rules) for the full built-in set.
...
- The written `$schema` always points at a file that exists. With `@wastech-mdlint/cli` installed, it is a relative path to that package's `schema.json`, computed from the config's own directory (so a config under `docs/` gets `../node_modules/…`). With nothing installed — the ordinary `npx` case — `init` generates a project-local `schema.json` next to the config and points at that instead. Custom rules force that same project-local schema regardless, since the built-in one cannot describe them. No remote URL is ever emitted.
- **The draft names the schema file, not just the config.** The `npx` case above writes a second file into your repository, so the draft carries a `Schema:` line with the ref the config will hold and, when one has to be generated, the filename and which of the two reasons applies — before you confirm, not only in the write summary afterwards. The write summary repeats it with the outcome.
...
### The rule set `init` proposes, and why your first `lint` is green

`init` decides two separate things about each rule, and it measures both against **the corpus the config it is writing will lint** — never a sample of it, and on a `merge` not your repository at all but the file set your existing `include` selects.

1. **Is the rule relevant?** Does that corpus contain the thing the rule inspects at all — relative links, heading anchors, images, tables, checklist items, placeholder sections, a shared ADR heading set? A rule with nothing to check is not proposed.
2. **Is the rule satisfied?** `init` runs every relevant rule over that same corpus once. One that reports nothing is written enabled, at its own default severity. One that reports something is still written down — as `"severity": "off"`, with the real count in the comment beside it.

So the first `lint` after `init` reports nothing, and the CI workflow `init` offers is green on the first push. That is the point of the policy rather than a side effect of it: a gate that reports hundreds of findings on the day it lands is ignored, then disabled, and takes the rules that were already clean down with it. The disabled entries are your work list, and each costs one word to enable:

```jsonc
"rules": [
  // Enabled: nothing to fix, so it can start failing the build today.
  { "rule": "REF-002" }, // The corpus contains 118 link(s) carrying a heading anchor; REF-002 checks that each anchor matches a real heading slug. It reports nothing over the 151 file(s) this config lints, so it is enabled.
  // Written down, switched off, with the number that switched it off.
  { "rule": "REF-001", "severity": "off" } // The corpus contains 1,204 relative link(s) to other files; REF-001 checks that every one resolves to a file in the corpus. It reports 16 finding(s) across 10 of the 151 file(s) this config lints, so it is written off rather than failing the first run; switch it on once those are cleared.
]
```

Every count in those comments is one you can reproduce: enable the rule and run `lint`. That is the whole reason inference measures the corpus instead of sampling it — a number in a permanent comment that was never true is worse than no number.

Two consequences worth stating plainly:

- **A green first run does not mean a clean repository.** It means nothing is enabled that would fail. What is wrong is in the config, next to the rule that would find it.
- **`init` reads every file the config will lint**, so it costs roughly one `lint` on top of the scan. It is a one-time command, and it is what makes the numbers in your config checkable.

`GRP-001` is the one rule whose comment can name something the config will _not_ report: it only reports a cycle spanning three or more documents, so an index and a member linking each other is cited as evidence and then explicitly excused. See [`GRP-001`](rules/GRP-001.md) for why that floor is where it is.
